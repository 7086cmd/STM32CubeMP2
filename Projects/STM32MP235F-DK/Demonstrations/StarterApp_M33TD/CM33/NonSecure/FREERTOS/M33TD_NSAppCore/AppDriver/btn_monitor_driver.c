 ...
 * - BUTTON_USER2 on GPIOC pin 11
 * - EXTI2 line 11 (EXTI2_11_IRQn)
 */

/**
  * @brief EXTI handle for USER2 button (EXTI2 line 11).
  *
  * @note The EXTI2_11 IRQ handler is expected to call HAL_EXTI_IRQHandler(&hexti2).
  */
...
static void BtnMonitorDriver_ExtiRisingCb(void)
{
  ...
  HAL_NVIC_DisableIRQ(EXTI2_11_IRQn);
...
static int BtnMonitorDriver_Init(BtnMonitorDriver_IRQCallback cb, void *context)
{
  ...
  /* Enable GPIOC clock (RCC resource 96 in this demo) */
  ...
  /* Acquire EXTI2 line 11 */
  if (RESMGR_STATUS_ACCESS_OK != ResMgr_Request(RESMGR_RESOURCE_RIF_EXTI2, RESMGR_EXTI_RESOURCE(11)))
  {
  ...
  /* Configure EXTI2 line 11 rising edge on GPIOC */
  EXTI_ConfigStructure.Line = EXTI2_LINE_11;
  ...
  HAL_NVIC_SetPriority(EXTI2_11_IRQn, configLIBRARY_MAX_SYSCALL_INTERRUPT_PRIORITY + DEFAULT_IRQ_PRIO, 0);
  HAL_NVIC_EnableIRQ(EXTI2_11_IRQn);
...
static int BtnMonitorDriver_DeInit(void)
{
  ...
  (void)ResMgr_Release(RESMGR_RESOURCE_RIF_EXTI2, RESMGR_EXTI_RESOURCE(11));
  (void)ResMgr_Release(RESMGR_RESOURCE_RIF_GPIOC, RESMGR_GPIO_PIN(11));
