...
void HAL_ETH_MspInit(ETH_HandleTypeDef *heth)
{
  ...
  /* Acquire GPIOA2 - ETH1.NRST using Resource manager */
  if (ResMgr_Request(RESMGR_RESOURCE_RIF_GPIOA, RESMGR_GPIO_PIN(2)) != RESMGR_STATUS_ACCESS_OK)
  {
    Error_Handler();
  }

  /* Acquire GPIOA9 - ETH1.MDC using Resource manager */
  if (ResMgr_Request(RESMGR_RESOURCE_RIF_GPIOA, RESMGR_GPIO_PIN(9)) != RESMGR_STATUS_ACCESS_OK)
  {
    Error_Handler();
  }

  /* Acquire GPIOA10 - ETH1.MDIO using Resource manager */
  if (ResMgr_Request(RESMGR_RESOURCE_RIF_GPIOA, RESMGR_GPIO_PIN(10)) != RESMGR_STATUS_ACCESS_OK)
  {
    Error_Handler();
  }

  /* Acquire GPIOA11 - ETH1.RX CTL using Resource manager */
  if (ResMgr_Request(RESMGR_RESOURCE_RIF_GPIOA, RESMGR_GPIO_PIN(11))  != RESMGR_STATUS_ACCESS_OK)
  {
    Error_Handler();
  }

  /* Acquire GPIOA13 - ETH1.TX CTL using Resource manager */
  if (ResMgr_Request(RESMGR_RESOURCE_RIF_GPIOA, RESMGR_GPIO_PIN(13)) != RESMGR_STATUS_ACCESS_OK)
  {
    Error_Handler();
  }

  /* Acquire GPIOA14 - ETH1.RX CLK using Resource manager */
  if (ResMgr_Request(RESMGR_RESOURCE_RIF_GPIOA, RESMGR_GPIO_PIN(14)) != RESMGR_STATUS_ACCESS_OK)
  {
    Error_Handler();
  }

  /* Acquire GPIOA15 - ETH1.TXD0 using Resource manager */
  if (ResMgr_Request(RESMGR_RESOURCE_RIF_GPIOA, RESMGR_GPIO_PIN(15)) != RESMGR_STATUS_ACCESS_OK)
  {
    Error_Handler();
  }

  /* Acquire GPIOC0 - ETH1.GTX.CLK using Resource manager */
  if (ResMgr_Request(RESMGR_RESOURCE_RIF_GPIOC, RESMGR_GPIO_PIN(0)) != RESMGR_STATUS_ACCESS_OK)
  {
    Error_Handler();
  }

  /* Acquire GPIOC1 - ETH1.TXD1 using Resource manager */
  if (ResMgr_Request(RESMGR_RESOURCE_RIF_GPIOC, RESMGR_GPIO_PIN(1)) != RESMGR_STATUS_ACCESS_OK)
  {
    Error_Handler();
  }

  /* Acquire GPIOC2 - ETH1.RXD1 using Resource manager */
  if (ResMgr_Request(RESMGR_RESOURCE_RIF_GPIOC, RESMGR_GPIO_PIN(2)) != RESMGR_STATUS_ACCESS_OK)
  {
    Error_Handler();
  }

  /* Acquire GPIOF1 - ETH1.RXD0 using Resource manager */
  if (ResMgr_Request(RESMGR_RESOURCE_RIF_GPIOF, RESMGR_GPIO_PIN(1)) != RESMGR_STATUS_ACCESS_OK)
  {
    Error_Handler();
  }

  /* Acquire GPIOH9 - ETH1.CLK 125 using Resource manager */
  if (ResMgr_Request(RESMGR_RESOURCE_RIF_GPIOH, RESMGR_GPIO_PIN(9)) != RESMGR_STATUS_ACCESS_OK)
  {
    Error_Handler();
  }

  /* Acquire GPIOH10 - ETH1.TXD2 using Resource manager */
  if (ResMgr_Request(RESMGR_RESOURCE_RIF_GPIOH, RESMGR_GPIO_PIN(10)) != RESMGR_STATUS_ACCESS_OK)
  {
    Error_Handler();
  }

  /* Acquire GPIOH11 - ETH1.TXD3 using Resource manager */
  if (ResMgr_Request(RESMGR_RESOURCE_RIF_GPIOH, RESMGR_GPIO_PIN(11)) != RESMGR_STATUS_ACCESS_OK)
  {
    Error_Handler();
  }

  /* Acquire GPIOH12 - ETH1.RXD2  using Resource manager */
  if (ResMgr_Request(RESMGR_RESOURCE_RIF_GPIOH, RESMGR_GPIO_PIN(12)) != RESMGR_STATUS_ACCESS_OK)
  {
    Error_Handler();
  }

  /* Acquire GPIOH13 - ETH1.RXD3  using Resource manager */
...
void HAL_ETH_MspDeInit(ETH_HandleTypeDef *heth)
{
  ...
  if (ResMgr_Release(RESMGR_RESOURCE_RIF_GPIOA, RESMGR_GPIO_PIN(2)) != RESMGR_STATUS_ACCESS_OK)
  {
    Error_Handler();
  }

  /* Release GPIOA9 using Resource manager */
  if (ResMgr_Release(RESMGR_RESOURCE_RIF_GPIOA, RESMGR_GPIO_PIN(9)) != RESMGR_STATUS_ACCESS_OK)
  {
    Error_Handler();
  }

  /* Release GPIOA10 using Resource manager */
  if (ResMgr_Release(RESMGR_RESOURCE_RIF_GPIOA, RESMGR_GPIO_PIN(10)) != RESMGR_STATUS_ACCESS_OK)
  {
    Error_Handler();
  }
